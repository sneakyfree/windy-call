...
#
# Voice billing is per-minute via Twilio. We charge a flat conservative
# estimate per call assuming up to ~4 minutes; if calls average
# longer, lift the value here. Inbound voice is "free" (Twilio bills
# us only for outbound paths and per-minute media); the inbound webhook
# returns TwiML and hangs up.
#
# voice.call         — outbound TTS call, ~$0.013/min × 4-min cap = $0.05
# voice.inbound      — inbound webhook, billing borne by caller's carrier
COSTS: dict[str, int] = {
    "voice.call":    50_000,   # $0.05  — outbound TwiML call (D.3)
    "voice.inbound":     0,    # caller's carrier pays; we don't charge agents
}

KEY_PREFIX = "cost:windy-call:passport"
